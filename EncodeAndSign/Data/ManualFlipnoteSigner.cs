...
//In the future i'd like to use it to provide standalone signing capabilities without having to generate a flipnote
//todo: implement this
//todo: batch conversion?
//I lost the code i was using to utilize this class
//rough explanation:
//extract the bytes from a flipnote that contain the hash signature, will be 0x80 null bytes in an unsigned flipnote
//patch together: bytes before the signature > signature > 0x10 null padding

namespace EncodeAndSign
{
    public class ManualFlipnoteSigner
    {

        public byte[] byteRSA { get; set; }
        public bool Signed { get; set; }


        public ManualFlipnoteSigner(string privateKey, byte[] Sign)
        {
            ...
            this.byteRSA = rsa.SignData(byteSign, CryptoConfig.MapNameToOID("SHA1"));

            this.Signed = rsa.VerifyData(byteSign, CryptoConfig.MapNameToOID("SHA1"), pemprivatekey);
