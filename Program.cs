            ...
            try
            {
                ...
                using (var pbar = new ProgressBar(totalTicks, "Flipnote Progress", options))
                {
                    ...
                    if (!File.Exists("frames/frame_0.png"))
                    {
                        ...
                        //todo: Stop running console commands sadge
                        Process framesplit = new Process();
                        ProcessStartInfo startInfo = new ProcessStartInfo();

                        startInfo.WindowStyle = System.Diagnostics.ProcessWindowStyle.Hidden;
                        startInfo.FileName = "cmd.exe";
                        startInfo.Arguments = @"/C cd ffmpeg/bin && ffmpeg -i ..\..\frames\input.mp4 -vf scale=256:192 ..\..\frames\frame_%d.png";
                        ...
                        pbar.Tick();
                        using (var child = pbar.Spawn(totalTicks, "Splitting Frames...", childOptions))
                        {
                            child.Tick(5);
                    ...
                    if (!File.Exists("frames/audio.wav"))
                    {
                        ...
                        //audioInfo.Arguments = @"/C cd ffmpeg/bin && ffmpeg -i ..\..\frames\input.mp4 -ac 1 -ar 8192 -acodec adpcm_ima_wav ..\..\frames\audio.wav";
                        ...
                        using (var child = pbar.Spawn(totalTicks, "Writing Wav...", childOptions))
                        {
                            ...
                            audio.WaitForExit();
                            audio.Dispose();
                            child.Tick(5, "Encoding Wav to ADPCM...");
                            using (Sox sox = new Sox("sox/sox.exe"))
                            {
                                sox.Output.Type = FileType.IMA;
                                sox.Output.ReverseNibbles = true;
                                sox.Output.SampleRate = 8192;

                                sox.Process("frames/audio.wav", "frames/audio.adpcm");
                            }
